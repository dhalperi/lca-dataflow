...
public class LCAStep extends PTransform<PCollectionTuple, PCollectionTuple> {
  ...
  public PCollectionTuple expand(PCollectionTuple input) {
    ...
    PCollectionView<Set<PaperPair>> knownAncestors =
        oldAncestors
                    ...
                    .of(
                        new DoFn<KV<Integer, CoGbkResult>, KV<PaperPair, Ancestor>>() {
                          ...
                          public void processElement(ProcessContext c) throws Exception {
                            // Key is the ancestor -- both delta and reachable vertices can reach
                            // it.
                            ...
                            for (Reachable r2 : join.getAll(keyedReachableTag)) {
                              for (Reachable r1 : delta) {
                                if (r1.dst == r2.dst) {
                                  // Skip delta join delta, which happens since reachable already
                                  // contains delta.
                                  continue;
                    ...
                    .of(
                        new DoFn<KV<Integer, CoGbkResult>, KV<Integer, Reachable>>() {
                          ...
                          public void processElement(ProcessContext c) throws Exception {
                            ...
                            Set<Reachable> newReachable =
                                Sets.newHashSet(join.getAll(keyedReachableTag));
                            Set<Reachable> newDelta = Sets.newHashSet();
                            // Record all the destinations this src can already.
                            for (Reachable r : newReachable) {
                              verify(
                                  alreadyKnown.add(r.dst),
                                  "Adding already known destination %s to src %s",
                                  r.dst,
                                  source);
                            }
