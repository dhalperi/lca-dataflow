...
public class CommonAncestors
    extends PTransform<PCollection<KV<Integer, Reachable>>, PCollection<KV<PaperPair, Ancestor>>> {
  ...
  private static class GenerateAncestors
      extends DoFn<KV<Integer, Iterable<Reachable>>, KV<PaperPair, Ancestor>> {
    private PCollectionView<Map<Integer, Integer>> papers;
      ...
      for (Reachable p1 : papers) {
        for (Reachable p2 : papers) {
          if (p1.dst < p2.dst) {
            c.output(
                KV.of(
                    PaperPair.of(p1.dst, p2.dst), Ancestor.of(ancestor, p1.depth, p2.depth, year)));
          }
        }
      }
    }
  ...
  public static void main(String[] args) {
    ...
    PCollectionView<Map<Integer, Integer>> papers =
        p.apply("LoadPapers", new LoadPapers("papers", options.getPapersFile()))
            .apply(View.asSingleton());

    PCollection<KV<Integer, Reachable>> reachable =
        p.apply("LoadReachable", new LoadReachable("reachable", options.getReachableFile()));
    ...
    reachable
        ...
        .apply(
            "OutputLCAs",
            TextIO.write().to(options.getOutputDirectory() + "/lcas").withSuffix(".txt"));
